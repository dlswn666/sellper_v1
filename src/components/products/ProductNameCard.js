    ...
    return (
        <Space direction="vertical" size="middle" style={{ display: 'block', width: '100%' }}>
            <Card hoverable style={{ width: '100%' }} onFocus={onCardFocus} tabIndex={0}>
                <Image.PreviewGroup items={data.images && data.images.length > 0 ? data.images : [defaultImage]}>
                ...
                <div style={{ marginTop: 16 }}>
                    <InputComponent ref={inputRef} placeholder="상품 검색 키워드를 입력해주세요" onBlur={fnOnBlur} />
                </div>
            </Card>
        </Space>
    );
